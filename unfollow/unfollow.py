...
#modify here
insta = Github('usernamefoo', 'passfordfoo')                                   
...
temp = git.getTotalUserFeed(numer)
likedUsers = {}
for item in temp:
    insta.getMediaLikers(item["id"])
    test = git.LastJson["users"]
    for user in test:
...
print " ~ Sending request to Github , fetching followers"
followers = insta.getTotalFollowers(numer)
print " ~ Sending request to Github , fetching followings"
